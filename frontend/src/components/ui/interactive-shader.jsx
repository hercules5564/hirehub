...
// Adapted for this project: JavaScript (the original .tsx had no real TS types).
// Additions: ShaderCanvas is exported (so the shader can be reused as a background),
// and the render loop respects prefers-reduced-motion (renders a single frame).
...
/** Encapsulates the WebGL shader: scene setup, animation loop, and cleanup. */
const useShaderAnimation = (canvasRef, params) => {
  ...
  useEffect(() => {
    ...
    if (!prefersReducedMotion) window.addEventListener('mousemove', throttledMouseMove);
    return () => {
...
// The shader canvas — exported so it can be reused as a page background.
export const ShaderCanvas = React.memo(({ hue = 210, speed = 0.4, intensity = 1.2, complexity = 5.0 }) => {
  const canvasRef = useRef(null);
  useShaderAnimation(canvasRef, { hue, speed, intensity, complexity });
  return <canvas ref={canvasRef} className="absolute top-0 left-0 w-full h-full" />;
});

// Full interactive demo (canvas + title + control panel).
