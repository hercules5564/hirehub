...
// Adapted for this project: JavaScript (not TS) + the app's dark theme instead
// of the original's --background/--sky-300 CSS-variable system (which would
// clash with this project's @theme tokens). Fixes vs the original:
//   - fragment shader uniform renamed pointLightPosition -> pointLightPos so it
//     matches the uniforms object and the mouse-tracking light actually lights
//     the mesh (the original set a uniform the shader never read).
//   - color accepts a real value (THREE.Color can't parse `hsl(var(--x))`).
//   - prefers-reduced-motion renders a single static frame.
//   - ResizeObserver handles container resize (not just window), and WebGL
//     resources are disposed on unmount.

export function GenerativeArtScene({ color = "#818cf8" }) {
    ...
    >
      ...
      <div className="relative z-20 flex flex-col items-center justify-end h-full pb-20 md:pb-32 text-center">
        <div className="max-w-3xl px-4 animate-fade-in">
          <h1 className="text-sm font-mono tracking-widest text-indigo-300/80 uppercase">
            {title}
          </h1>
